...
Auth: `Authorization: Bearer <device token>`. Tokens are created in the web app (`/app/settings/tokens`), stored by the CLI in `~/.hubskillz/config.json` (`{ "baseUrl", "token", "machineId" }`). Route handlers live in `apps/web/src/app/api/cli/*`. Zod schemas for every payload live in `packages/shared/src/cli/*` and are imported by both sides.
